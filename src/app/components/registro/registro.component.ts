...
export class RegistroComponent implements OnInit {
  mensaje: string = 'Bienvenido, para completar el registro hay que rellenar todos los datos.';
  error: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this.formPaciente = this.fb.group({
      ...
      nombre: new FormControl(null, [Validators.required, Validators.minLength(2), Validators.maxLength(20), this.palabraValidator()]),
      apellido: new FormControl(null, [Validators.required, Validators.minLength(2), Validators.maxLength(20), this.palabraValidator()]),
      edad: new FormControl(null, [Validators.required, Validators.min(1), Validators.max(120)]),
      dni: new FormControl(null, [Validators.required, Validators.pattern(/^\d+$/), Validators.minLength(7), Validators.maxLength(9)]),
      obraSocial: new FormControl(null, [Validators.required, Validators.minLength(2), Validators.maxLength(20), this.palabraValidator()]),
  ...
  palabraValidator(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      const valid = /^[a-zA-ZáéíóúÁÉÍÓÚñÑ'’-]+( [a-zA-ZáéíóúÁÉÍÓÚñÑ'’-]+)*$/.test(control.value);
  ...
  async registrar() {
    ...
    try {
      ...
      if (this.usuarioActual?.perfil === 'Administrador') {
        await this.authService.createUserWithoutLogin(datos.correo, datos.clave);
      ...
      try {
        ...
        this.error = true;
        this.mensaje = 'Hubo un error al guardar el usuario en la base de datos';
      }
    } catch (error: any) {
      this.loader.hide();
  ...
  cambiarFormulario(formulario: string) {
    ...
    this.error = false;
    this.mensaje = 'Bienvenido, para completar el registro hay que rellenar todos los datos.';
